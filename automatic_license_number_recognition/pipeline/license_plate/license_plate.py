...
class LicensePlateDetector:
	...
	def detectCharacterCandidates(self, region):
		...
		contours  = cv2.findContours(charCandidates.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		if len(contours) == 2:
			cnts = contours[0]
		else:
			cnts = contours[1]
	...
	def scissor(self, lp):
		...
		# _, cnts, _ = cv2.findContours(lp.candidates.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		contours  = cv2.findContours(lp.candidates.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		if len(contours) == 2:
			cnts = contours[0]
		else:
			cnts = contours[1]
	...
	def preprocessChar(char):
		...
		contours  = cv2.findContours(char.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		if len(contours) == 2:
			cnts = contours[0]
		else:
			cnts = contours[1]
